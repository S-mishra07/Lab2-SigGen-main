...
VL_INLINE_OPT void Vsigdelay___024root___sequent__TOP__0(Vsigdelay___024root* vlSelf) {
    ...
    // Init
    SData/*8:0*/ __Vdly__sigdelay__DOT____Vcellout__addrCounter__count;
    ...
    if (vlSelf->rd_enable) {
        vlSelf->dout = vlSelf->sigdelay__DOT__sigRam__DOT__ram_array
            [(1U & (IData)(vlSelf->sigdelay__DOT____Vcellout__addrCounter__count))];
    }
    if (vlSelf->wr_enable) {
...
void Vsigdelay___024root___eval_debug_assertions(Vsigdelay___024root* vlSelf) {
    ...
    if (VL_UNLIKELY((vlSelf->wr_enable & 0xfeU))) {
        Verilated::overWidthError("wr_enable");}
    if (VL_UNLIKELY((vlSelf->rd_enable & 0xfeU))) {
